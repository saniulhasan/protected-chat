    ...
    const startScreenSharing = async () => {
        try {
          const screenStream = await navigator.mediaDevices.getDisplayMedia({
            video: true, // You can customize options here
          });
      
          // Replace the current media stream with the screen-sharing stream
...
>
  {isScreenSharing ? (
    <>
      <i className="fa-solid fa-window" style={{ fontSize: '20px' }}></i>
      <span className="ml-2">Stop Sharing</span>
    </>
  ) : (
    <>
      <MdScreenShare/>
